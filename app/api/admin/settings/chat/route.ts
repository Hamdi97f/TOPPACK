...
import { normaliseChatSettings } from "@/lib/site-settings";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ chat: settings.chat });
  } catch (err) {
    return apiErrorResponse(err, "Échec du chargement des paramètres du chat");
  }
}
...
export async function PUT(req: Request) {
  ...
  const chat = normaliseChatSettings(body);
  try {
    ...
    return NextResponse.json({ chat: saved.chat });
  } catch (err) {
