...
import { normaliseWinSmsSettings } from "@/lib/site-settings";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ winsms: settings.winsms });
  } catch (err) {
    return apiErrorResponse(err, "Échec du chargement des paramètres WinSMS");
  }
}
...
export async function PUT(req: Request) {
  ...
  const winsms = normaliseWinSmsSettings(body);
  try {
    ...
    return NextResponse.json({ winsms: saved.winsms });
  } catch (err) {
