...
import { normaliseBrandingSettings } from "@/lib/site-settings";
...
export async function GET() {
  ...
  try {
    ...
    return apiErrorResponse(err, "Échec du chargement de l'identité du site");
  }
}
...
export async function PUT(req: Request) {
  ...
  const branding = normaliseBrandingSettings(body);
  try {
    const current = await apiClient.getSiteSettings(session.user.apiToken);
    const saved = await apiClient.setSiteSettings(session.user.apiToken, { ...current, branding });
