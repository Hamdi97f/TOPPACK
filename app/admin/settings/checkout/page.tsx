import { redirect } from "next/navigation";
import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import { apiClient } from "@/lib/api-client";
import { CheckoutSettingsForm } from "@/components/admin/CheckoutSettingsForm";

export const dynamic = "force-dynamic";

export default async function CheckoutSettingsPage() {
  const session = await getServerSession(authOptions);
  if (!session || session.user.role !== "ADMIN") redirect("/login?callbackUrl=/admin/settings/checkout");
  const settings = await apiClient.getCheckoutSettings(session.user.apiToken);

  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold text-kraft-900 mb-2">Paramètres du formulaire de commande</h1>
      <p className="text-sm text-kraft-700 mb-4">
        Choisissez les champs à afficher sur la page de validation de commande, et lesquels sont obligatoires.
        Au moins un mode de paiement doit rester activé.
      </p>
      <CheckoutSettingsForm initial={settings} />
    </div>
  );
}
