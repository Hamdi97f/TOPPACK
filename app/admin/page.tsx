...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: "Chiffre d'affaires (30j)", value: formatPrice(revenue30d) },
    { label: "Produits actifs", value: adapted.filter((p) => p.isActive).length },
    { label: "Produits en stock faible", value: lowStock.length },
  ];

  return (
    <div>
      <h1 className="text-2xl font-bold text-kraft-900 mb-6">Tableau de bord</h1>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mt-8">
        <section className="card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/admin/orders" className="text-sm text-kraft-700 hover:underline">Voir toutes</Link>
          </div>
          {recent.length === 0 ? (
            <div className="text-sm text-kraft-600">Aucune commande pour le moment.</div>
          ) : (
        ...
        <section className="card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/admin/products" className="text-sm text-kraft-700 hover:underline">Gérer</Link>
          </div>
          {lowStock.length === 0 ? (
            <div className="text-sm text-kraft-600">Tous les produits ont un stock suffisant.</div>
          ) : (
            <ul className="divide-y divide-kraft-100 text-sm">
              {lowStock.map((p) => (
                <li key={p.id} className="py-2 flex justify-between">
                  <Link href={`/admin/products/${p.id}`} className="hover:text-kraft-700">{p.name}</Link>
                  <span className="text-red-600 font-semibold">{p.stock} restants</span>
                </li>
              ))}
            </ul>
          )}
